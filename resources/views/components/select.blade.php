...
<div class="space-y-1.5">
    @if($label)
        <label for="{{ $selectId }}" class="block text-sm font-semibold text-secondary">
            {{ $label }}
            @if($required)
    ...
    <select
        ...
        {{ $attributes->merge(['class' => '
            marketplace-field appearance-none bg-no-repeat
            ' . ($hasError
                ? 'border-danger bg-red-50 text-red-950 focus:border-danger focus:ring-danger/15'
                : ''
            )
        ']) }}
